...
async function generatePDF() {
  ...
  function drawHeader() {
    ...
    doc.setFontSize(10);
    doc.rect(15, y, 85, 30);
    doc.text(`Patient Name : ${name.value}`, 18, y + 7);
    doc.text(`Age / Gender : ${age.value} / ${sex.value}`, 18, y + 14);
    doc.text(`UAID / Lab Ref : ${uaid.value}`, 18, y + 21);
    doc.text(`SIN No. : ${sin_no.value}`, 18, y + 28);

    doc.rect(110, y, 85, 30);
    doc.text(`Sample Collection Time : ${sample_collection_time.value}`, 113, y + 7);
    doc.text(`Sample Received Time : ${sample_received_time.value}`, 113, y + 14);
    doc.text(`Reported Time : ${reported_time.value}`, 113, y + 21);
    doc.text(`Ref. Doctor : ${ref_doctor.value}`, 113, y + 28);
  ...
  if (ref_doctor.value) {
    doc.text(`(${ref_doctor.value})`, 15, pageHeight - 28);
  }
